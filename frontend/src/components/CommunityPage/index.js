...
function CommunityPage() {
  ...
  const { communityPosts, singlePost } = useSelector((state) => state.posts);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (windowHeight + scrollTop >= documentHeight - threshold) {
        ...
        if (id) dispatch(postsActions.thunkGetCommunityPosts(id, currentPage)); // Fetch posts for the specified page
      }
    }
  ...
  useEffect(() => {
    ...
    if (id) dispatch(postsActions.thunkGetCommunityPosts(id, page)); // Fetch posts for the specified page
  ...
  ePost = Object.values(communityPosts).reverse().sort((a, b) => a.createdAt - b.createdAt).filter((p) => singleCommunity.id === p.communityId)
       ...
       { style ? <div style={{
        ...
        <div className="community-page-content">
            <div className="posts">
                ...
                <div style={{ backgroundColor: "#EDEFF1"}} onClick={(() => history.push(`/communities/recent${singleCommunity.id}/:page`))}id="best">
                <i style={{ color: `${singleCommunity.CommunityStyle?.base}`}} class="fi fi-sr-bahai"></i>
                <p style={{ color: `${singleCommunity.CommunityStyle?.base}`}}>New</p>
                </div>
