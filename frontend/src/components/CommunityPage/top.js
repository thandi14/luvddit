...
function TopCommunityPage() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (windowHeight + scrollTop >= documentHeight - threshold) {
        ...
        if (id) dispatch(postsActions.thunkGetTopCommunityPosts(id, currentPage)); // Fetch posts for the specified page
      }
    }
  ...
  useEffect(() => {
    ...
    if (id) dispatch(postsActions.thunkGetTopCommunityPosts(id, page)); // Fetch posts for the specified page
  ...
  ePost = Object.values(topCommunityPosts).reverse().sort((a, b) => b.Votes.filter((v) => v.upVote === 1).length - a.Votes.filter((v) => v.upVote === 1).length).filter((p) => singleCommunity.id === p.communityId)
       ...
       { style ? <div style={{
        ...
        <div className="community-page-content">
            <div className="posts">
                ...
                <div id="filter-side1">
                <div onClick={(() => history.push(`/communities/${singleCommunity.id}/:page`))} id="best">
                ...
                <div style={{ backgroundColor: "#EDEFF1"}} onClick={(() => history.push(`/communities/top/${singleCommunity.id}/:page`))} id="best">
                <i style={{ color: `${singleCommunity.CommunityStyle?.base}`}} class="fi fi-ss-signal-bars-good"></i>
                <p style={{ color: `${singleCommunity.CommunityStyle?.base}`}}>Top</p>
                </div>
...
export default TopCommunityPage
