...
function YourHotProfilePage() {
    ...
    useEffect(() => {
      ...
      const handleScroll = () => {
        ...
        if (windowHeight + scrollTop >= documentHeight - threshold) {
          ...
          dispatch(postsActions.thunkGetHotOverview(user?.id, currentPage)); // Fetch posts for the specified page
      }
      }


    useEffect(() => {
        dispatch(postsActions.thunkGetHotOverview(user?.id, page)); // Fetch posts for the specified page
    }, [dispatch, page]);
   ...
   // let ePosts = Object.values(posts).reverse()
    ...
    let filterdPosts = Object.values(postsHotOverview).filter((p) => p.userId === user?.id || p.Comments.some((c) => c.userId === user?.id))
  ...
  useEffect(() => {
    ...
    <div className="posts">
        ...
        <div style={{ backgroundColor: "#EDEFF1"}} onClick={(() => history.push("/profile/hot/:page"))} id="best">
...
export default YourHotProfilePage
