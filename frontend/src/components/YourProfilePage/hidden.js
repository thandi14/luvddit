...
function HiddenPosts() {
    ...
    const handleHidden = async (id) => {
        if (singlePost.PostSetting && !singlePost.PostSetting.hidden) await dispatch(postsActions.thunkUpdateSaved(id))
        else if (!singlePost.PostSetting) await dispatch(postsActions.thunkCreateSaved(id))
      }

      const handleUnhidden = async (id) => {
            ...
            <div id="comment5">
                ...
                <div id={editMenu}>
                   ...
                   { !post.PostSetting || !post.PostSetting.saved ? <p onClick={(() => {
                      handleHidden(post.id)
                    })}>
                    <i class="fi fi-rr-bookmark"></i>Save</p> :
                    <p onClick={(() => {
                      handleUnhidden(post.id)
                    })}>
