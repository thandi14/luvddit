...
function Commented2Posts() {
    ...
    useEffect(() => {
      ...
      const handleScroll = () => {
        ...
        if (windowHeight + scrollTop >= documentHeight - threshold) {
          ...
          dispatch(postsActions.thunkGetComments(id, currentPage)); // Fetch posts for the specified page
        }
      }
  ...
  useEffect(() => {
   ...
   // if (!ePost.length) return <h1 className="data-not-here"></h1>
    ...
    <div id="filter-side1">
        <div style={{ backgroundColor: "#EDEFF1"}} onClick={(() => history.push(`/profile2/${id}/comments/:page`))} id="best">
        ...
        {filterdPosts?.map((post, i) =>
            ...
            <span onClick={((e) => {
                e.stopPropagation()
            })}className="userName2">{other.username} </span>
...
export default Commented2Posts
