...
function Commented2TopPosts() {
    const { posts, singlePost, postsTopComments } = useSelector((state) => state.posts);
    ...
    useEffect(() => {
      ...
      const handleScroll = () => {
        ...
        if (windowHeight + scrollTop >= documentHeight - threshold) {
          ...
          dispatch(postsActions.thunkGetTopComments(id, currentPage)); // Fetch posts for the specified page
        }
      }
  ...
  useEffect(() => {
    dispatch(postsActions.thunkGetTopComments(id, page)); // Fetch posts for the specified page
  }, [dispatch, page, id]);

    let filterdPosts = Object.values(postsTopComments)
  ...
  useEffect(() => {
   ...
   // if (!ePost.length) return <h1 className="data-not-here"></h1>
    ...
    <div id="filter-side1">
        ...
        <div onClick={(() => history.push(`/profile2/${id}/comments/hot/:page`))}id="best">
        <i class="fi fi-rs-flame"></i>
        <p>Hot</p>
        </div>
        <div style={{ backgroundColor: "#EDEFF1"}} onClick={(() => history.push(`/profile2/${id}/comments/top/:page`))} id="best">
...
export default Commented2TopPosts
