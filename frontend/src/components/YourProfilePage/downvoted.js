...
function DownvotedPosts() {
    ...
    filterdPosts = filterdPosts.filter((p) => p.Votes.some((v) => v.userId === user.id && v.downVote === 1))

    filterdPosts.forEach((p) => {
        p.Votes.forEach((v) => {
            let voteDate = new Date(v.updatedAt)
            v.updatedAt = Date.parse(voteDate)
        })
        p.Votes.sort((a, b) => {
            return b.updatedAt - a.updatedAt
        })

            p.updatedAt = p.Votes[0].updatedAt
  ...
  useEffect(() => {
   ...
   // if (!ePost.length) return <h1 className="data-not-here"></h1>
    ...
    <div id="aHeader3">
        ...
        <span onClick={(() => history.push("/profile/posts"))} id="aH8">POSTS</span>
        ...
        <span onClick={(() => history.push("/profile/downvoted"))}id="aHl">DOWNVOTED</span>
    ...
    <div className="posts5">
        <div></div>
        {!filterdPosts.length ? <NoPosts name="downvoted anything" /> : filterdPosts?.map((post, i) =>
