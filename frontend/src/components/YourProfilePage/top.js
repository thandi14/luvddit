...
function YourTopProfilePage() {
    ...
    useEffect(() => {
      ...
      const handleScroll = () => {
        ...
        if (windowHeight + scrollTop >= documentHeight - threshold) {
          ...
          dispatch(postsActions.thunkGetTopOverview(user?.id, currentPage)); // Fetch posts for the specified page
      }
      }


    useEffect(() => {
        dispatch(postsActions.thunkGetTopOverview(user?.id, page)); // Fetch posts for the specified page
    }, [dispatch, page]);
   ...
   let filterdPosts = Object.values(postsTopOverview).filter((p) => p.userId === user?.id || p.Comments.some((c) => c.userId === user?.id))
  ...
  useEffect(() => {
    ...
    <div className="posts">
        ...
        <div id="filter-side1">
        <div onClick={(() => history.push("/profile/:page"))} id="best">
        ...
        <div style={{ backgroundColor: "#EDEFF1"}} onClick={(() => history.push("/profile/top/:page"))} id="best">
...
export default YourTopProfilePage
