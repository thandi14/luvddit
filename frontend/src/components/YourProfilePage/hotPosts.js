...
function UsersHotPosts() {
    ...
    useEffect(() => {
      ...
      const handleScroll = () => {
        ...
        if (windowHeight + scrollTop >= documentHeight - threshold) {
          ...
          dispatch(postsActions.thunkGetUserHotPosts(user?.id, currentPage)); // Fetch posts for the specified page
        }
      }
    ...
    useEffect(() => {
        dispatch(postsActions.thunkGetUserHotPosts(user?.id, page)); // Fetch posts for the specified page
    }, [dispatch, page]);
  ...
  useEffect(() => {
   ...
   // if (!ePost.length) return <h1 className="data-not-here"></h1>
    ...
    <div id="filter-side1">
        <div onClick={(() => history.push("/profile/posts/:page"))} id="best">
        <i class="fi fi-rr-bahai"></i>
        <p>New</p>
        </div>
        <div style={{ backgroundColor: "#EDEFF1"}} onClick={(() => history.push("/profile/posts/hot/:page"))}id="best">
