...
function TopPage() {
  ...
  const clearButton = () => {
    ...
    Cookies.set(`${user.id}`, clearTime, { expires: 365 });
};
...
useEffect(() => {
  ...
  useEffect(() => {
    async function fetchData() {
      await dispatch(postsActions.thunkGetTopPosts(page))
      }
      fetchData()
    ...
    return (
        <div className="splashPage">
            <div className="posts">
                ...
                <div style={{ backgroundColor: "#EDEFF1"}} onClick={(() => {
                  window.alert("/top")
                })} id="best">
                <i
                 style={{ color: '#0079D3'}}
                class="fi fi-ss-signal-bars-good"></i>
                <p
                style={{ color: '#0079D3'}}
                >
                  Top</p>
